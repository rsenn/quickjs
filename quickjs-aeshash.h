...
aeshash8(const uint8_t* s, size_t len, uint32_t h) {
  ...
  return _mm_extract_epi32(H, 0) ^ _mm_extract_epi32(H, 1) ^ _mm_extract_epi32(H, 2) ^
...
aeshash8_alt(const uint8_t* s, size_t len, uint32_t h) {
  ...
  // horizontally xor the 4 32-bit parts of the 128-bit register
  return _mm_extract_epi32(H, 0) ^ _mm_extract_epi32(H, 1) ^ _mm_extract_epi32(H, 2) ^ _mm_extract_epi32(H,
3);
}*/
