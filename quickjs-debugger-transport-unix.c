...
js_debugger_connect(JSContext* ctx, const char* address) {
  ...
  data->handle = client;
  js_debugger_attach(
      ctx, js_transport_read, js_transport_write, js_transport_peek, js_transport_close, data);
}
...
js_debugger_wait_connection(JSContext* ctx, const char* address) {
  ...
  int reuseAddress = 1;
  assert(setsockopt(server, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuseAddress, sizeof(reuseAddress)) >=
         0);
