...
static int
js_transport_write_fully(JSDebuggerInfo* info, const char* buffer, size_t length) {
  int offset = 0;
  while(offset < length) {
...
static int
js_transport_write_message_newline(JSDebuggerInfo* info, const char* value, size_t len) {
...
static int
js_transport_write_value(JSDebuggerInfo* info, JSValue value) {
...
static JSValue
js_transport_new_envelope(JSDebuggerInfo* info, const char* type) {
...
static int
js_transport_send_event(JSDebuggerInfo* info, JSValue event) {
...
static JSValue
js_get_scopes(JSContext* ctx, int frame) {
...
static int
js_debugger_get_frame(JSContext* ctx, JSValue args) {
...
static void
js_send_stopped_event(JSDebuggerInfo* info, const char* reason) {
...
static void
js_process_breakpoints(JSDebuggerInfo* info, JSValue message) {
  JSContext* ctx = info->ctx;
...
JSValue
js_debugger_file_breakpoints(JSContext* ctx, const char* path) {
...
static int
js_process_debugger_messages(JSDebuggerInfo* info, const uint8_t* cur_pc) {
  ...
  do {
...

    {
      ...
      if((type = JS_ToCString(ctx, JS_GetPropertyStr(ctx, message, "type")))) {
...
void
js_debugger_exception(JSContext* ctx) {
  JSDebuggerInfo* info = js_debugger_info(JS_GetRuntime(ctx));
...
static void
js_debugger_context_event(JSContext* caller_ctx, const char* reason) {
...
void
js_debugger_new_context(JSContext* ctx) {
  js_debugger_context_event(ctx, "new");
}

void
js_debugger_free_context(JSContext* ctx) {
...
void
js_debugger_check(JSContext* ctx, const uint8_t* cur_pc) {
  JSDebuggerInfo* info = js_debugger_info(JS_GetRuntime(ctx));
  if(info->is_debugging)
    return;
  ...
  if(info->stepping) {
    ...
    if(info->step_depth == depth && location.filename == info->step_over.filename && location.line == info->step_over.line && location.column == info->step_over.column)
      goto done;
  }
  ...
  if(at_breakpoint) {
    ...
    if(info->stepping == JS_DEBUGGER_STEP_CONTINUE) {
      ...
      if((location.filename == info->step_over.filename && location.line == info->step_over.line && location.column == info->step_over.column) || js_debugger_stack_depth(ctx) > info->step_depth)
        goto done;
      info->stepping = 0;
      info->is_paused = 1;
...
void
js_debugger_free(JSRuntime* rt, JSDebuggerInfo* info) {
...
void
js_debugger_attach(JSContext* ctx, size_t (*transport_read)(void* udata, char* buffer, size_t length), size_t (*transport_write)(void* udata, const char* buffer, size_t length), size_t (*transport_peek)(void* udata), void (*transport_close)(JSRuntime* rt, void* udata), void* udata) {
...
int
js_debugger_is_transport_connected(JSRuntime* rt) {
  return js_debugger_info(rt)->transport_close != NULL;
}

void
js_debugger_cooperate(JSContext* ctx) {
